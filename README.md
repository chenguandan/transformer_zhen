# Transformer Translation Model for zh2en
...
```
set VOCAB_FILE=./data/vocab.zh.32768
set VOCAB_FILE_EN=./data/vocab.en.32768
set DATA_DIR=./data
...
linux下的命令为
```
VOCAB_FILE=./data/vocab.zh.32768
VOCAB_FILE_EN=./data/vocab.en.32768
DATA_DIR=./data
MODEL_DIR=./model_zhen
PARAM_SET=base
python transformer_main.py --data_dir=$DATA_DIR --model_dir=$MODEL_DIR \
    --vocab_file=$VOCAB_FILE --vocab_file_en=$VOCAB_FILE_EN --param_set=$PARAM_SET \
    --bleu_source=./data/translate_zhen_raw/um_corpus_test_ch.txt --bleu_ref=./data/translate_zhen_raw/um_corpus_test_en.txt
```
如果需要使用其他训练语料需要注意：
- model/model_params.py中指定了vocab_size,vocab_size_en，需要进行修改为data/vocab*.txt中的行数
- 数据集形式为一行一个样本，双语分别用一个文件存储
- utils/schedule.py 指定了NUM_EXAMPLES作为训练语料、开发集的大小
