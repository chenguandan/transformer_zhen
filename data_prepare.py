...
VOCAB_FILE = "vocab.zh.%d" % _TARGET_VOCAB_SIZE
VOCAB_FILE_en = "vocab.en.%d" % _TARGET_VOCAB_SIZE
...
def encode_and_save_files(
        subtokenizer, subtokenizer_en,
        data_dir, raw_files, tag, total_shards):
    ...
    for counter, (input_line, target_line) in enumerate(zip(
        ...
        example = dict_to_example(
            {"inputs": subtokenizer.encode(input_line, add_eos=True, is_ch=True),
             "targets": subtokenizer_en.encode(target_line, add_eos=True)})
...
def main(unused_argv):
    ...
    train_files = ['casict-A_ch.txt','casict-B_ch.txt' ]
    train_files_en = ['casict-A_en.txt','casict-B_en.txt' ]
    def add_dir(files):
        return [os.path.join(FLAGS.raw_dir, file) for file in files]
    train_files = add_dir(train_files)
    train_files_en = add_dir(train_files_en)
    ...
    vocab_file_en = os.path.join(FLAGS.data_dir, VOCAB_FILE_en)
    #zh
    subtokenizer = tokenizer.Subtokenizer.init_from_files(
        vocab_file, train_files, _TARGET_VOCAB_SIZE, _TARGET_THRESHOLD,
        min_count=None if FLAGS.search else _TRAIN_DATA_MIN_COUNT,
        is_zh=True)
    #en
    subtokenizer_en = tokenizer.Subtokenizer.init_from_files(
        vocab_file_en, train_files_en, _TARGET_VOCAB_SIZE, _TARGET_THRESHOLD,
        min_count=None if FLAGS.search else _TRAIN_DATA_MIN_COUNT)

    #合并文件
    tf.logging.info("Step 3/4: Compiling training and evaluation data")
    train_files_dict = {"inputs":train_files, "targets":train_files_en}
    compiled_train_files = compile_files(FLAGS.raw_dir, train_files_dict, _TRAIN_TAG)
    eval_files = {"inputs": [os.path.join(FLAGS.raw_dir, 'um_corpus_test_ch.txt')],
                  "targets": [os.path.join(FLAGS.raw_dir, 'um_corpus_test_en.txt')]}
    ...
    train_tfrecord_files = encode_and_save_files(
        subtokenizer, subtokenizer_en,
        FLAGS.data_dir, compiled_train_files, _TRAIN_TAG,
        _TRAIN_SHARDS)
    encode_and_save_files(
        subtokenizer, subtokenizer_en, FLAGS.data_dir, compiled_eval_files, _EVAL_TAG,
        _EVAL_SHARDS)
...
def define_data_prepare_flags():
    ...
    flags.DEFINE_string(
        name="raw_dir", short_name="rd", default=r"./data/translate_zhen_raw",
        help=
...
def main_dev(unused_args):
    ...
    # en
    subtokenizer_en = tokenizer.Subtokenizer(vocab_file_en)
