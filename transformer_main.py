...
# import . as official.transformer
import sys
...
def model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope("model"):
        ...
        xentropy, weights = metrics.padded_cross_entropy_loss(
            logits, targets, params["label_smoothing"], params["vocab_size_en"])
...
def run_loop(
        ...
        bleu_source=None, bleu_ref=None, bleu_threshold=None, vocab_file=None, vocab_file_target=None):
    ...
    **Step vs. Epoch vs. Iteration**
    ...
    for i in xrange(schedule_manager.train_eval_iterations):
        ...
        #TODO 这里的evaluation有个corrupted record at 0的BUG
        eval_results = estimator.evaluate(
            input_fn=dataset.eval_input_fn,
            steps=None)#schedule_manager.single_iteration_eval_steps)
        ...
        if evaluate_bleu:
            uncased_score, cased_score = evaluate_and_log_bleu(
                estimator, bleu_source, bleu_ref, vocab_file, vocab_file_target)
...
def define_transformer_flags():
    ...
    flags.DEFINE_integer(
        name="train_steps", short_name="ts", default=None,#10000,#2M*10/2048(batch_size)=10000
        help="The number of steps used to train.")
    flags.DEFINE_integer(
        name="train_epochs", short_name="te", default=40,
        help="The number of epochs used to train.")

    flags.DEFINE_integer(
        name="steps_between_evals", short_name="sbe", default=10,
        help=
            "The Number of training steps to run between evaluations. This is "
            "used if --train_steps is defined.")
    flags.DEFINE_integer(
        name="epochs_between_evals", short_name="epochs_between_evals", default=1,
        help=
        "The Number of training epcohs to run between evaluations. This is "
        "used if --train_epochs is defined.")

    flags.DEFINE_integer(
        name="batch_size", short_name="batch_size", default=None,
        help=
        "batch size")

    flags.DEFINE_integer(
        name="num_parallel_calls", short_name="num_parallel_calls", default=1,
        help=
        "parallel calls to read data")
    ...
    flags.DEFINE_string(
        ...
        help=
            ...
            "the vocab file.")
    flags.DEFINE_string(
        name="vocab_file_en", short_name="vfe", default=None,
        help=
        "Path to subtoken vocabulary file of English. If data_download.py was used to "
        "download and encode the training data, look in the data_dir to find "
        "the vocab file.")
    flags.DEFINE_string(
        name="data_dir", short_name="data_dir", default=None,
        help=
        "data dir")
    flags.DEFINE_string(
        name="model_dir", short_name="model_dir", default=None,
        help=
        "model path")
    flags.DEFINE_string(
        name="export_dir", short_name="export_dir", default='./data/export_model',
        help=
        "model export dir")
    flags.DEFINE_float(name="stop_threshold", default=None,help="bleu score threshold to stop")
