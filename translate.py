...
def _encode_and_add_eos(line, subtokenizer, is_ch=False):
  """Encode line with subtokenizer, and add EOS id to the end."""
  return subtokenizer.encode(line, is_ch=is_ch) + [tokenizer.EOS_ID]
...
def translate_file(
    estimator, subtokenizer, subtokenizer_target, input_file, output_file=None,
  ...
  def input_generator():
    ...
    for i, line in enumerate(sorted_inputs):
      ...
      yield _encode_and_add_eos(line, subtokenizer, is_ch=True)
...
def translate_text(estimator, subtokenizer, subtokenizer_target, txt):
...
def main(unused_argv):
  ...
  if FLAGS.text is not None:
    ...
    translate_text(estimator, subtokenizer, subtokenizer_target, FLAGS.text)
...
def define_translate_flags():
  ...
  flags.DEFINE_string(
      ...
      help=
      "Path to target subtoken vocabulary file. If data_download.py was used to "
      "download and encode the training data, look in the data_dir to find "
      "the vocab file.")
