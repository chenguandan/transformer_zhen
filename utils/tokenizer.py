...
class Subtokenizer(object):
    ...
    def init_from_files(
            ...
            file_byte_limit=1e6, reserved_tokens=None, is_zh=False):
        ...
        if tf.gfile.Exists(vocab_file):
            ...
            token_counts = _count_tokens(files, file_byte_limit, is_zh)
    ...
    def encode(self, raw_string, add_eos=False, is_ch=False):
        """Encodes a string into a list of int subtoken ids."""
        ret = []
        raw_string = _native_to_unicode(raw_string)
        if is_ch:
...
def _count_tokens(files, file_byte_limit=1e6, is_zh=False):
